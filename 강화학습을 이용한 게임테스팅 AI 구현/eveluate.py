...
class DQN(base_agent.BaseAgent):
    ...
    def train(self, env, seed, gamma, max_minutes, max_episodes, goal_mean_100_reward):
        ...
        print('Final evaluation score {:.2f}\u00B1{:.2f} in {:.2f}s training time,\n'.format(final_eval_score, score_std, training_time ))
        env.close();
        del env
...
def main(args):
    ...
    for seed in SEEDS:
        ...
        with sc2_env.SC2Env(map_name=MAPNAME, players=players, \
            ...
            # DQN 신경망 훈련
            result, final_eval_score, training_time = agent.train(env, seed, gamma, max_minutes,
    ...
    dqn_x =200 # DQN 결과와 비교를 위해 스케일 맞춤
    ...
    # 차트 제목
    axs[0].set_title('default reward (Training)')
    axs[1].set_title('default reward (Evaluation)')
    plt.xlabel('Episodes')
